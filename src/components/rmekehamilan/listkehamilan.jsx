...
const ListRiwayatKehamilan = () => {
   ...
   useEffect(() => {
    const getAllByRm = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/rmekehamilan/list/${id}`);
            ...
            <div className='card-content p-2'>  
                ...
                <thead style={{ textAlign: 'center' }}>
                    <tr>
                        ...
                        <th className='header1'>Nama Suami</th>
                ...
                <tbody>
                    {data.map((item) => 
                        ...
                        <td>{item.namePasien}</td>
                        <td>{item.nameSuami}</td>
                        <td>
                            <Link to={`/rmekehamilan/edit/list/${item.nomorRmKehamilan}/${item.id}`}>
...
export default ListRiwayatKehamilan;